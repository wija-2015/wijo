...
.factory('ManagerCollabs', function($http,$window) {
  var promise;
  var ManagerCollabs = {
    findCollabs: function() {
      if ( !promise ) {
         promise = $http.get("http://localhost:8181/managers/collabs/"+$window.sessionStorage.idUser).then(function (response) {
          console.log(response);
          return response.data;
        });
      }
